...
export async function fetchLanguageUsagePerDay(timeframe: Timeframe): Promise<LanguageUsagePerDay[]> {
  return fetchJson(`${API_BASE}/language-usage-per-day?timeframe=${timeframe}`);
}
...
export async function fetchSeatInfo(timeframe: Timeframe): Promise<SeatInfo> {
  return fetchJson(`${API_BASE}/seat-info?timeframe=${timeframe}`);
}
