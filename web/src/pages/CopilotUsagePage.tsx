...
export function CopilotUsagePage() {
  ...
  const { data: languageUsagePerDay, isLoading: langDayLoading, refetch: refetchLangDay } = useLanguageUsagePerDay(timeframe);
  ...
  // Language usage per day colors - vibrant colors matching Model usage per day style
  const LANGUAGE_COLORS: Record<string, { color: string; dashed: boolean }> = {
    // Purple/indigo tones (like Claude models)
    'TypeScript': { color: '#8b5cf6', dashed: true },      // Purple dashed
    'tsx': { color: '#7c3aed', dashed: true },             // Violet dashed
    'Python': { color: '#a855f7', dashed: true },          // Bright purple dashed
    'python': { color: '#a855f7', dashed: true },          // Bright purple dashed
    // Green/teal tones (like GPT models)  
    'JavaScript': { color: '#22c55e', dashed: false },     // Green solid
    'javascript': { color: '#22c55e', dashed: false },     // Green solid
    'Java': { color: '#10b981', dashed: true },            // Emerald dashed
    'java': { color: '#10b981', dashed: true },            // Emerald dashed
    'Go': { color: '#14b8a6', dashed: false },             // Teal solid
    'go': { color: '#14b8a6', dashed: false },             // Teal solid
    // Amber/orange tones (like Gemini models)
    'C#': { color: '#f59e0b', dashed: false },             // Amber solid
    'csharp': { color: '#f59e0b', dashed: true },          // Amber dashed
    'Ruby': { color: '#ef4444', dashed: false },           // Red solid
    'ruby': { color: '#ef4444', dashed: false },           // Red solid
    'Rust': { color: '#f97316', dashed: true },            // Orange dashed
    'rust': { color: '#f97316', dashed: true },            // Orange dashed
    'PHP': { color: '#ec4899', dashed: false },            // Pink solid
    'php': { color: '#ec4899', dashed: false },            // Pink solid
    // Common file types - still colorful
    'markdown': { color: '#06b6d4', dashed: true },        // Cyan dashed
    'yaml': { color: '#0ea5e9', dashed: false },           // Sky blue solid
    'json': { color: '#3b82f6', dashed: true },            // Blue dashed
    // Other - use warm amber/brown (not grey)
    'Other languages': { color: '#d97706', dashed: false }, // Dark amber solid
  };

  // Sort languages by total usage for consistent ordering
  const sortedLanguageKeys = useMemo(() => {
    if (!languageUsagePerDay || languageUsagePerDay.length === 0) return [];
    const totals: Record<string, number> = {};
    for (const day of languageUsagePerDay) {
      for (const [key, val] of Object.entries(day)) {
        if (key !== 'date' && typeof val === 'number') {
          totals[key] = (totals[key] || 0) + val;
        }
  ...
  // Areas config for stacked area chart (Language usage per day)
  // Vibrant fallback colors cycling through purple, green, amber spectrum
  const FALLBACK_COLORS = ['#8b5cf6', '#22c55e', '#f59e0b', '#06b6d4', '#ef4444', '#a855f7', '#10b981', '#f97316'];
  const languageAreas = sortedLanguageKeys.map((key, index) => {
    const config = LANGUAGE_COLORS[key] || { 
  ...
  return (
    <div className="max-w-7xl">
      ...
      {/* 7. Language usage per day */}
      <div className="mb-4">
        <StackedAreaChartCard
          title="Language usage per day"
          subtitle="Daily breakdown of language usage in requests across all chat modes and code completions"
          data={languageUsagePerDay || []}
          areas={languageAreas}
          yAxisLabel="%"
          yAxisDomain={[0, 100]}
          showPercentage={true}
          isLoading={langDayLoading}
        />
      </div>
