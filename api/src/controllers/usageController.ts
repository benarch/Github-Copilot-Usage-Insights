...
export async function getModelUsagePerDay(timeframe: Timeframe): Promise<ModelUsagePerDay[]> {
  ...
  const result = Array.from(dailyMap.values()).map(dayData => {
    ...
    const percentages: ModelUsagePerDay = { date };
    for (const [key, count] of Object.entries(counts)) {
      percentages[key] = total > 0 ? Math.round((count / total) * 10000) / 100 : 0;
    }
    return percentages;
  });
...
// Get language usage per day as percentages
export async function getLanguageUsagePerDay(timeframe: Timeframe): Promise<Array<{ date: string; [language: string]: string | number }>> {
  const days = parseInt(timeframe);
  const { startDate, endDate } = getDateRange(days);
  ...
  if (await shouldUseDirectJson()) {
    const records = await jsonReader.getAllRecords();
    const filtered = jsonReader.filterByDateRange(records, startDate, endDate);
    return jsonReader.getLanguageUsagePerDayFromRecords(filtered);
  }

  // For SQLite, aggregate language usage per day
  const db = getDatabase();
  const rows = db.prepare(`
    SELECT 
      uud.day as date,
    ...
    GROUP BY uud.day, ulm.language
    ORDER BY uud.day ASC
  `).all(startDate, endDate) as Array<{ date: string; language: string; count: number }>;
  ...
  // Build daily data with counts
  const dailyData = new Map<string, { [lang: string]: number }>();
  for (const row of rows) {
    if (!dailyData.has(row.date)) {
    ...
    const dayData = dailyData.get(row.date)!;
    const category = topLanguages.includes(row.language) ? row.language : 'Other languages';
    dayData[category] = (dayData[category] || 0) + row.count;
  }
...
export async function getSeatInfo(timeframe: Timeframe): Promise<SeatInfo> {
  const days = parseInt(timeframe);
  const { startDate, endDate } = getDateRange(days);
  ...
  if (await shouldUseDirectJson()) {
    ...
    // Get all unique users from all records (represents total seats)
    const allUniqueUsers = new Set<number>();
    for (const record of records) {
      allUniqueUsers.add(record.user_id);
    }
    
    // Get active users in the timeframe
    const activeUsers = new Set<number>();
    for (const record of filtered) {
      activeUsers.add(record.user_id);
    }
    
    const totalSeats = allUniqueUsers.size;
    const activeSeats = activeUsers.size;
    const unusedSeats = totalSeats - activeSeats;
    const utilizationRate = totalSeats > 0 ? Math.round((activeSeats / totalSeats) * 100) : 0;
    
    return { totalSeats, activeSeats, unusedSeats, utilizationRate };
  }

  const db = getDatabase();
  
  // Get total unique users (all time - represents total seats allocated)
  const totalSeatsRow = db.prepare(`
    SELECT COUNT(DISTINCT user_id) as total FROM user_usage_details
  `).get() as { total: number };
  ...
  const activeSeatsRow = db.prepare(`
    ...
    FROM user_usage_details 
    WHERE day BETWEEN ? AND ?
  `).get(startDate, endDate) as { active: number };
  
  const totalSeats = totalSeatsRow.total;
  const activeSeats = activeSeatsRow.active;
  const unusedSeats = totalSeats - activeSeats;
  const utilizationRate = totalSeats > 0 ? Math.round((activeSeats / totalSeats) * 100) : 0;
